...
pub fn spawn_tile_colliders(
    ...
    let mut level_to_tile_coords: HashMap<Entity, HashSet<GridCoords>> = HashMap::new();

    tile_query.for_each(|(&grid_coords, parent)| {
        if let Ok(grandparent) = parent_query.get(parent.get()) {
            level_to_tile_coords
                    ...
                    for tile_rect in tile_rects {
                        level
                            ...
                            .insert(Transform::from_xyz(
                                (tile_rect.left + tile_rect.right + 1) as f32 * grid_size as f32
                                    / 2.,
                                (tile_rect.bottom + tile_rect.top + 1) as f32 * grid_size as f32
                                    / 2.,
                                0.,
                            ))
